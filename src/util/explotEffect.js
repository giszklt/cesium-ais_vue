...
export class explotEffect {
	constructor(viewer, option) {
		...
		this.entity = this.viewer.entities.add({
			...
			position: Cesium.Cartesian3.fromDegrees(
				option.lon, option.lat, option.alt
			),
		});
		this.init();
	}
	...
	init() {
		...
		var particleSystem = this.scene.primitives.add(
			new Cesium.ParticleSystem({
				image: 'data/boom.png',//生成所需粒子的图片路径
				//粒子在生命周期开始时的颜色
	...
	preUpdateEvent() {
		...
		this.viewer.scene.preUpdate.addEventListener(function (scene, time) {
			// _this.particleSystem.show = Cesium.JulianDate.secondsDifference(_this.time, time) >= 0 ? true : false ;
			//发射器地理位置
